...
class TuneKstandard_old():
    def __init__(self, dataset, use_toy=True):
        self.data = dataset
        # full_dummies.shape
        self.dummies = articles.ArticleFeaturesSimpleFeatures(self.data.articles, use_article_id=True).x

        # Only keep customers at train_x and train_y
        train_x_customer_ids_set = set(self.data.train_x.customer_id)
        self.customers_at_y = train_x_customer_ids_set.intersection(set(self.data.train_y.customer_id))    # toy=539
        self.customers_at_vy = train_x_customer_ids_set.intersection(set(self.data.train_vy.customer_id))   # toy=607
        self.threshold = 300
        if use_toy:
            self.threshold = 50
    ...
    def try_multiple_k(self, k_list):
        ...
        scores_test = []
        for k in k_list:
            model = models.KnnRecommender(self.data, self.dummies, groups=k, threshold=self.threshold)
        ...
        return {'k': k_list, 'map_at_k_validation': scores_validation_float, 'map_at_k_test': scores_test_float}
...
if __name__ == '__main__':
    ...
    features1 = articles.ArticleFeaturesSimpleFeatures(toy.articles, use_article_id=True)
    # features2 = articles.ArticleFeaturesSimpleFeaturesSubset1(toy.articles, use_article_id=True)
